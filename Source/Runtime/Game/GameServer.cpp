...
namespace game
{
    ...
    const char *gameident() { return "HevEn"; }
}
...
namespace server
{
    ...
    void setupdemoplayback()
    {
        ...
        {
            lilswap(&hdr.version, 2);
            if(hdr.version!=DEMO_VERSION) formatstring(msg, "demo \"%s\" requires an %s version of HevEn", file, hdr.version<DEMO_VERSION ? "older" : "newer");
            else if(hdr.protocol!=PROTOCOL_VERSION) formatstring(msg, "demo \"%s\" requires an %s version of HevEn", file, hdr.protocol<PROTOCOL_VERSION ? "older" : "newer");
        }
        if(msg[0])
        {
            DELETEP(demoplayback);
    ...
    int laninfoport() { return GAME_LANINFO_PORT; }
    int serverport() { return GAME_SERVER_PORT; }
    const char *defaultmaster() { return ""; } //No Master
    int masterport() { return GAME_MASTER_PORT; }
