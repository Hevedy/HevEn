...
// ---------------------------------------------------------------------------
//  File:			Sound.cpp
//  Description: 	Basic positional sound using sdl_mixer
// ---------------------------------------------------------------------------
//  Log:			Source.
//
//
...
void initmumble()
{
    ...
    if(VALID_MUMBLELINK) return;
        ...
        if(mumblelink)
        {
            ...
            if(mumbleinfo) wcsncpy(mumbleinfo->name, L"HevEn", 256);
        }
    #elif defined(_POSIX_SHARED_MEMORY_OBJECTS)
        ...
        if(mumblelink >= 0)
        {
            ...
            if(mumbleinfo != (MumbleInfo *)-1) wcsncpy(mumbleinfo->name, L"HevEn", 256);
        }
    #endif
