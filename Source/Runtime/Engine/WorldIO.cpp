...
// ---------------------------------------------------------------------------
//  File:			WorldIO.cpp
//  Description: 	Loading & saving of levels and savegames
// ---------------------------------------------------------------------------
//  Log:			Source.
//
//
...
{
    if(f->read(&hdr, 3*sizeof(int)) != 3*sizeof(int)) { conoutf(CON_ERROR, "level %s has malformatted header", ogzname); return false; }
    lilswap(&hdr.version, 2);

    if(!memcmp(hdr.magic, "TMAP", 4))
    {
        if(hdr.version>MAPVERSION) { conoutf(CON_ERROR, "level %s requires a newer version of HevEn", ogzname); return false; }
        if(f->read(&hdr.worldsize, 6*sizeof(int)) != 6*sizeof(int)) { conoutf(CON_ERROR, "level %s has malformatted header", ogzname); return false; }
        lilswap(&hdr.worldsize, 6);
        if(hdr.worldsize <= 0|| hdr.numents < 0) { conoutf(CON_ERROR, "level %s has malformatted header", ogzname); return false; }
    }
    else if(!memcmp(hdr.magic, "OCTA", 4))
    {
        if(hdr.version!=OCTAVERSION) { conoutf(CON_ERROR, "level %s uses an unsupported level format version", ogzname); return false; }
        if(f->read(&ohdr.worldsize, 7*sizeof(int)) != 7*sizeof(int)) { conoutf(CON_ERROR, "level %s has malformatted header", ogzname); return false; }
        lilswap(&ohdr.worldsize, 7);
        if(ohdr.worldsize <= 0|| ohdr.numents < 0) { conoutf(CON_ERROR, "level %s has malformatted header", ogzname); return false; }
    ...
    else { conoutf(CON_ERROR, "level %s uses an unsupported level type", ogzname); return false; }
...
{
    defformatstring(ogzname, "Game/Levels/%s/%s.hel", fname); // HevEn Level
    path(ogzname);
    ...
    {
        samegame = false;
        conoutf(CON_WARN, "WARNING: loading level from %s game, ignoring entities except for lights/mapmodels", gametype);
    }
...
bool load_world(const char *mname, const char *cname)        // still supports all map formats that have existed since the earliest cube betas!
{
    ...
    execfile("Engine/Core/Managers/Level_Content.hec", false);
